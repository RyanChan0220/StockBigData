...
def trans_db_multi(db_name, trans_path):
    global g_trans_path
    global g_db_name
    g_trans_path = trans_path
    g_db_name = db_name
    for root1, dirs1, files1 in os.walk(trans_path):
        pool = ThreadPool(fork_processing)
        ret = pool.map(trans_stock2db, dirs1)
        pool.close()
        pool.join()


def trans_stock2db(stock_name):
    global g_db_name
    global g_trans_path
    mysql = MySQL(g_db_name)
    mysql.connect()
    col_type = list()
    col_type.append("`ID` INT NOT NULL AUTO_INCREMENT")
    col_type.append("`DEAL_DATE` DATETIME NULL")
    ...
    stock_path = g_trans_path + "\\" + stock_name
    try:
